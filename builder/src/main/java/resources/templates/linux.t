...
<STOP>
<FONT_LOAD>
  bOk = gslc_FontAdd(&m_gui,$<FONT_ID>,$<FONT_REFTYPE>,$<FONT_REF>,$<FONT_SZ>);
...
<IMAGE>
  ...
  pElemRef = gslc_ElemCreateImg(&m_gui,$<WIDGET_ENUM>,$<PAGE_ENUM>,(gslc_tsRect){$<X>,$<Y>,$<WIDTH>,$<HEIGHT>},
    gslc_GetImageFromFile(m_strImgPath$<ID>,$<IMAGE_FORMAT>));

<STOP>
<IMGBUTTON>
  
  // Initialize $<WIDGET_ENUM> button with image label
  // - Extra code to for path generation based on location of executable
  strncpy(m_strImgBtnPath$<ID>,strPath,MAX_PATH);
  strncat(m_strImgBtnPath$<ID>,$<IMAGE_DEFINE>,MAX_PATH);
  strncpy(m_strImgBtnSelPath$<ID>,strPath,MAX_PATH);
  strncat(m_strImgBtnSelPath$<ID>,$<IMAGE_SEL_DEFINE>,MAX_PATH);
  pElemRef = gslc_ElemCreateBtnImg(&m_gui,$<WIDGET_ENUM>,$<PAGE_ENUM>,(gslc_tsRect){$<X>,$<Y>,$<WIDTH>,$<HEIGHT>},
          gslc_GetImageFromFile(m_strImgBtnPath$<ID>,$<IMAGE_FORMAT>),
          gslc_GetImageFromFile(m_strImgBtnSelPath$<ID>,$<IMAGE_FORMAT>),
          &CbBtnCommon);
...
<TEXTBOX>
  ...
  pElemRef = gslc_ElemXSliderCreate(&m_gui,E_SCROLLBAR$<ID>,$<PAGE_ENUM>,&m_sXSlider[$<COUNT>],
          (gslc_tsRect){$<X>+$<WIDTH>-21,$<Y>+4,20,$<HEIGHT>-8},0,$<ROWS>,0,5,true);
  gslc_ElemSetCol(&m_gui,pElemRef,$<FRAME_COLOR>,$<FILL_COLOR>,$<GLOW_COLOR>);
  gslc_ElemXSliderSetPosFunc(&m_gui,pElemRef,&CbSlidePos);
<STOP>
<TEXT_COLOR>
