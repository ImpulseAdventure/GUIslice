...
//   - A touch activity timer watches for 5 seconds of
//     inactivity, which can be used to blank the screen.
//     A callback function is invoked upon any touch event.
...
#define SCREENBLANK_TIMEOUT 5000 // Milliseconds before screen blank
bool    m_bScreenBlankActive = false;
int32_t m_nScreenBlankLastTouch = 0;
...
{
  if (m_bScreenBlankActive) {
    Serial.print("ScreenBlank: end due to touch\n");
    // Enable the screen
    m_bScreenBlankActive = false;
  }
  
  // Reset timer
  m_nScreenBlankLastTouch = millis();
  gslc_SetTouchTrigFunc(&m_gui,&CbTouchTrig);
}

// This routine is called periodically to see if we have
// waited longer than the timeout for a touch event. If
// the timeout occurs, a screen blank event can be triggered
// and a wakeup callback is requested for the next touch
// event.
// NOTE: no screen blanking will occur in this example.
//       The user can replace the debug messaging with
//       the appropriate hardware control.

void CheckTouchTimeout()
{
  if (m_bScreenBlankActive) {
    // Screen already blanked, no need to check timer
    return;
  }
  if (millis()-m_nScreenBlankLastTouch >= SCREENBLANK_TIMEOUT) {
    // Blank the screen
    m_bScreenBlankActive = true;
    // Register the touch wakeup callback
    Serial.print("ScreenBlank: start due to timeout\n");
    gslc_SetTouchTrigFunc(&m_gui,&CbTouchTrig);
  }
}
